      ...
      >
        <tr v-for="(item, index) in chargeItemList" :key="index">
          ...
          <td class="text-primary">
            {{ item.chargeItemGas ? item.chargeItemGas : '-' }}
          </td>
          <td>
            {{ item.useGasMoney }}{{ accountDuction(item.useGasMoney, item.chargeItemMoney) }}
          </td>
          <td
          ...
          <td>
            <el-button
              v-if="item.chargeItemSourceCode !== 'SCENEFEE'"
              type="danger"
              plain
...
export default {
  ...
  computed: {
    waiteAppendItemListCopy() {
      return JSON.parse(JSON.stringify(this.waiteAppendItemList))
    },
    // 添加按钮禁用与否
    canAddNew: function () {
  ...
  methods: {
    ...
    //用气金额后面的账户抵扣：用气金额-欠费金额
    accountDuction(useGasMoney, chargeItemMoney) {
      const result = BN.minus(useGasMoney, chargeItemMoney)
      if (result != 0) {
        return `（账户抵扣：${result}）`
      }
    },
