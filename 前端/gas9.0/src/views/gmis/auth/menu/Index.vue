<template>
  <div class="menu">
    <el-row :gutter="10">
      <el-col :sm="6" :xs="24">
        <div class="app-container" style="padding-right: 5px">
          <search-box>
            <el-input v-model="label"
                      :placeholder="$t('table.menu.label')"
                      clearable @keyup.enter.native="search"
                      style="width: 150px"
            />

            <el-tooltip class="item" content="新增/删除时，请先勾选菜单" effect="dark" placement="top">
              <el-button v-has-permission="['menu:add']" type="success" @click="add">
                {{ $t('table.add') }}
              </el-button>
          ...
          <commonTree ref="menuTree" style="height: calc(100vh - 193px); overflow-y: scroll" :tree-data="menuTree"
                      @nodeClick="nodeClick"
          >
          ...
          <div>
            <el-form ref="form" :model="menu" :rules="rules" label-position="right" label-width="100px">
              ...
              <el-form-item :label="$t('table.status')" prop="isEnable">
                <el-radio-group v-model="menu.isEnable">
      ...
      <el-col :sm="10" :xs="24">
        <el-card class="box-card">
          <div class="app-container">
            <search-box>
              <div slot="left">
                <el-input v-model="resourceQueryParams.model.code" :placeholder="$t('table.resource.code')"
                          clearable
                          style="width: 100px"
                />
                <el-input v-model="resourceQueryParams.model.name" :placeholder="$t('table.resource.name')"
                          clearable
                          style="width: 100px"
                />
                <el-button type="primary" @click="resourceSearch">{{ $t('table.search') }}
                </el-button>
              </div>
              <div slot="right">
                <el-button
    ...
    <resource-edit ref="resourceEdit" :dialog-visible="dialog.isVisible" :type="dialog.type"
...
<script>
  import commonTree from '@/components/gmis/CommonTree.vue'
      ...
      menuComponent() {
        ...
        if (this.menu.path && this.menu.path !== '/') {
          ...
          if (isUrl) {
            ...
            // comp = `前端组件：src/views/gmis${this.menu.path}/Index.vue`
            comp = `组件路径：src/views/${this.menu.component}.vue`
          }
        } else {
          comp = `组件路径：src/views/gmis/Index.vue`
        }
        return comp
      }
    },
    ...
    methods: {
      ...
      computedComponent() {
        ...
        if (isUrl) {
          ...
          if (this.menu.path) {
            this.menu.component = `gmis${this.menu.path}/Index`
          } else {
            this.menu.component = `gmis/Index`
          }
        }
      },
      initMenuTree() {
