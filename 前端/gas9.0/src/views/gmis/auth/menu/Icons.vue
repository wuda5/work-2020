<template>
  <el-dialog :close-on-click-modal="false" :title="$t('table.menu.icon')" :visible.sync="isVisible" :width="width" top="50px">
    <el-input v-model="name" class="filter-item search-item" clearable placeholder="icon" @keyup.native="searchIcon" />
    <el-tabs v-model="activeName" style="margin-top: -1rem;">
      <el-tab-pane label="SVG" name="svg">
        <ul>
          <li v-for="icon of svgIcons" :key="icon" class="svg-item">
            <span :class="{'active':activeIndex === icon}">
              <svg-icon :icon-class="icon" class-name="icon" @click.native="chooseIcon(icon)" @dblclick.native="confirm" />
            </span>
            <p>{{ icon }}</p>
          </li>
        </ul>
