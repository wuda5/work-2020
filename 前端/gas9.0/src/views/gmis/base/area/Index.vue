    ...
    <el-row :gutter="10">
      <el-col :sm="12" :xs="24">
        ...
        <div class="treeBox">
          <search-box>
            <div slot="left">
              <el-input v-model="label" :placeholder="$t('table.area.label')" />
        ...
        <el-card class="box-card">
          ...
          <div>
            <el-form
              ...
              label-width="100px"
            >
              <el-form-item :hidden="true" :label="$t('table.area.parentId')" prop="parentId">
                <el-tooltip
              ...
              <el-form-item :label="$t('table.area.parentId')" prop="parentLabel">
                <el-input v-model="area.parentLabel" readonly disabled="disabled" />
              </el-form-item>
              ...
              <el-form-item :label="$t('table.area.level')" prop="level">
                <el-radio-group v-model="area.level.key" border="true" size="small">
                  <el-radio-button
                    v-for="(item, key, index) in dicts.AREA_LEVEL"
                    :key="index"
                    :label="item"
                    :value="key"
...
export default {
  ...
  methods: {
    ...
    loadTree(node, resolve) {
      areaApi.query({ parentId: node.data.id || 0 }).then(response => {
        const res = response.data
        resolve(res.data)
      })
    },
