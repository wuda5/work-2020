  ...
  <div>
    ...
    <search-box>
      ...
      <el-select v-model="userSearch.model.useGasTypeId" clearable placeholder="用气类型">
        <el-option v-for="item in dict.GAS_TYPE" :key="item.id" :label="item.name" :value="item.id"></el-option>
      </el-select>
      <!-- 气表类型 -->
      ...
      <!-- 安装地址 -->
      <el-input v-model="userSearch.model.gasMeterAddress" clearable placeholder="安装地址" />
    ...
    <Pagination
      ...
      :page="userSearch.current"
      :limit="userSearch.size"
      :total="total"
    />
  </div>
</template>
