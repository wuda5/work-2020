...
<template>
  <el-card shadow="never">
    <div slot="header" class="clearfix">
      <search-box>
        <div slot="left">
          <span class="card-title">用气类型统计</span>
        </div>
        <div slot="right">
  ...
  methods: {
    ...
    getChart2Data() {
      ...
      return [
        ...
        {
          '参数': '金额',
          '本月': this.total(this.typeValue, 'feiAmount', data.nowDate),
          '上月': this.total(this.typeValue, 'feiAmount', data.lastMonthDate),
          '去年同期': this.total(this.typeValue, 'feiAmount', data.lastYearDate)
        }
      ]
    },
