...
<template>
  <el-card shadow="never" class="counter">
    <div slot="header" class="clearfix">
      <search-box>
        <div slot="left">
          <span class="card-title">发票张数统计（合计票数 {{ totalNum }} 张）</span>
        </div>
        <div slot="right">
  ...
  methods: {
    getData() {
      ...
      tjApi.SinvoiceCount(params).then(res => {
        ...
        if (isSuccess) {
          ...
          this.totalNum = data.searchDate.totalNum
          ...
          for (let key in data.searchDate) {
            if (typeof data.searchDate[key] === 'object') {
              ...
              itemList.kindList.forEach(item => {
                itemObj[item.ktName] = item.amount
              })
              rows1.push(itemObj)
            }
          }
    ...
    getChart2Data() {
      ...
      return [
        {
          发票: '红票',
          本月: this.total(this.typeValue, data.nowDate, 0, 'amount'),
          上月: this.total(this.typeValue, data.lastMonthDate, 0, 'amount'),
          去年同期: this.total(this.typeValue, data.lastYearDate, 0, 'amount'),
        },
        {
          发票: '蓝票',
          本月: this.total(this.typeValue, data.nowDate, 1, 'amount'),
          上月: this.total(this.typeValue, data.lastMonthDate, 1, 'amount'),
          去年同期: this.total(this.typeValue, data.lastYearDate, 1, 'amount'),
        },
        {
          发票: '废票',
          本月: this.total(this.typeValue, data.nowDate, 2, 'amount'),
          上月: this.total(this.typeValue, data.lastMonthDate, 2, 'amount'),
          去年同期: this.total(this.typeValue, data.lastYearDate, 2, 'amount'),
        },
      ]
    },
      ...
      // 未筛选
        ...
        for (let key in data) {
          if (typeof data[key] === 'object') {
            let kindList = data[key].kindList
            total += parseInt(kindList[index][fileKey])
          }
        }
        return total
      }
