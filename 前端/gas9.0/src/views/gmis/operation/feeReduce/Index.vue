<!-- 缴费减免活动 -->
<template>
  <div class="qc-main feeReduce-page">
    <el-card shadow="never" class="no-border">
      <div slot="header">
        <span class="card-title">{{ $t('operation.preferential.feeReduce.title') }}</span>
      </div>
      <!-- 列表搜索区域 -->
      <search-box class="search-box operation-box">
        <div slot="left" class="">
          <el-form :ref="searchFormRef" :model="searchForm" :inline="true" class="qc-search-form">
            ...
            <el-form-item label="" prop="activityName">
              <el-input v-model="searchForm.activityName" clearable :placeholder="$t('operation.preferential.feeReduce.activityName')">
              </el-input>
            </el-form-item>
            <!-- 操作 -->
            <el-form-item label="" prop="dataStatus">
              <el-button type="primary" name="feeReduce-search" icon="el-icon-search" class="search-btn feeReduce-search" @click.native="handleSearch">{{ $t('search') }}</el-button>
              <el-button type="warning" name="feeReduce-search" icon="el-icon-refresh-left" class="search-reset-btn" @click.native="resetSearch(searchFormRef)">{{ $t('reset') }}</el-button>
            </el-form-item>
            <!-- <el-form-item label="">
        ...
        <div slot="right" class="operation-btn-box">
          <el-button v-has-permission="['feeReduce:add']" class="add-btn" name="add-btn" type="success" @click.native="showFormDialog(formStatusField.ADD, null)">{{ $t('add') }}</el-button>
        </div>
      </search-box>
      <div>
        <!--数据-->
        ...
        >
          ...
          <el-table-column
            prop="activityName"
            :label="$t('operation.preferential.feeReduce.activityName')"
          >
          </el-table-column>
          <!-- 收费场景 -->
          <el-table-column
            prop="tollItemScene"
            :label="$t('operation.preferential.feeReduce.tollItemScene')"
          >
            <template #default="{ row }">
              {{ tollItemSceneMap[row.tollItemScene] }}
            </template>
          </el-table-column>
          <!-- 收费项 -->
          <el-table-column
            prop="tollItemName"
            :label="$t('operation.preferential.feeReduce.tollItemName')"
          >
          </el-table-column>
          <!-- 活动金额方式 -->
          <el-table-column
            prop="activityMoneyType"
            :label="$t('operation.preferential.feeReduce.activityMoneyTypeLabel')"
          >
          ...
          <!-- 开始时间 -->
          <el-table-column 
            :label="$t('operation.preferential.feeReduce.startTime')" 
            prop="startTime"
          >
            <template #default="scope">
          ...
          <!-- 结束时间 -->
          <el-table-column 
            :label="$t('operation.preferential.feeReduce.endTime')" 
            prop="endTime"
          >
            <template #default="scope">
          ...
          >
            <template #default="{ row }">
              <el-button v-has-permission="['feeReduce:edit']" type="primary" name="row-edit" @click.native="showFormDialog(formStatusField.EDIT, row)">{{ $t('edit') }}</el-button>
              <!-- <el-button v-has-permission="['feeReduce:delete']" type="danger" name="row-delete" @click.native="handleDelete(row)">{{ $t('delete') }}</el-button> -->
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
        ...
        <qc-dialog
          :width="600"
          :title="currentRow ? $t('operation.preferential.feeReduce.edit') : $t('operation.preferential.feeReduce.add')"
          :visible="formDialogVisible"
          data-key="formDialogVisible"
          @close="dialogClose"
        >
          <fee-reduce-form v-if="formDialogVisible" :form-data="currentRow" @success="handleSaveSuccess"></fee-reduce-form>
        </qc-dialog>
      </div>
    </el-card>
  </div>
...
import FeeReduceForm from './Form.vue'
...
export default {
  ...
  methods: {
    ...
    async handleDelete(row) {
      const confirmResult = await awaitToJs(this.$confirm(this.$t('operation.preferential.feeReduce.deleteConfirm'), this.$t('common.tips'), {
        type: 'warning'
      }))
    ...
    async handleSwitchChange(val, row) {
      ...
      if (!dataStatus) {
        const confirmResult = await awaitToJs(this.$confirm(this.$t('operation.preferential.feeReduce.disableConfirm'), this.$t('common.tips'), {
          type: 'warning'
        }))
        if (confirmResult.error) {
    ...
    useGasTypesFormat(value, activityScene) {
      return this.IS_RECHARGE_GIVE(activityScene) ? JSON.parse(value).use_gas_type_name.join('、') : '--'
    },
