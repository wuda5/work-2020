<!-- 采暖价格表单 -->
<template>
  <div v-loading="loading" class="form-page price-form-page HeatingPriceForm-page">
    <!-- 表单区域 -->
    <el-form :ref="formRef" :model="form" :rules="formRules" label-position="top" :inline="true" class="qc-form disabled-form clearfix price-form" :class="{ 'view-price-form': onlyView }">
      <!-- 循环显示采暖和非采暖的阶梯设置表格表单 -->
      <el-row :gutter="15" class="price-ladder-row">
        <template v-for="(value, key) in isHeatingMap">
          <el-col v-if="showLadderPrice(key)" :key="key" class="price-ladder-col" :class="[isHeatingPrice(key) ? 'heating-price-col' : 'noe-heating-price-col']" :span="onlyView ? 24 : 12">
            <el-card class="" shadow="never">
              <search-box class="">
                <div slot="left" class="">
                  <div class="table-title">{{ isHeatingPrice(key) ? $t('operation.gasPrice.heatingPricePlan') : $t('operation.gasPrice.nonHeatingPricePlan') }}</div>
                </div>
              </search-box>
              <!-- 阶梯单价和气量 -->
                ...
                >
                  ...
                  >
                    <template #default="{ row, $index }">
                      <el-form-item :key="row.gas.key" :prop="`${isHeatingPrice(key) ? 'heatingLadders' : 'nonHeatingLadders'}[${$index}].gas.value`" :rules="setLadderGasRules(row.gas, $index, generateLadders(key))">
                  ...
                  >
                    <template #default="{ row, $index }">
                      <el-form-item :key="row.price.key" :prop="`${isHeatingPrice(key) ? 'heatingLadders' : 'nonHeatingLadders'}[${$index}].price.value`" :rules="setLadderPriceRules(row.price, $index, generateLadders(key))">
              ...
              <!-- 采暖价格切换时间 -->
              <el-form-item v-if="isHeatingPrice(key)" class="cycleStartTime-form-item" prop="heatingcycleStartTime" :label="$t('operation.gasPrice.cycleStartTime')">
                <el-date-picker v-model="form.heatingcycleStartTime" value-format="yyyy-MM-dd" :disabled="disabled" :picker-options="startTimePickerOptions" name="startTime" :placeholder="$t('operation.gasPrice.cycleStartTime')"></el-date-picker>
              </el-form-item>
              <!-- 非采暖价格切换时间 -->
              <el-form-item v-else class="cycleStartTime-form-item" prop="nonHeatingcycleStartTime" :label="$t('operation.gasPrice.cycleStartTime')">
                <el-date-picker v-model="form.nonHeatingcycleStartTime" value-format="yyyy-MM-dd" :disabled="disabled" :picker-options="startTimePickerOptions" name="startTime" :placeholder="$t('operation.gasPrice.cycleStartTime')"></el-date-picker>
              </el-form-item>
            </el-card>
          </el-col>
        </template>
...
import { isHeatingMap,  } from '@/enums/gasPrice'
...
import { saveHeatingPrice } from '@/api/operation/gasPrice'

const ladderNumer = 6 // 价格阶梯数量，预留7阶

export default {
  name: 'HeatingPriceForm',
  components: {
  ...
  data() {
    ...
    this.isHeatingMap = isHeatingMap
    // 生成初始化阶梯相关数据
    const laddres = new Array(ladderNumer).fill('').map((v, k) => ({ gas: { value: v, key: `gas${k + 1}`, field: `gas${k + 1}`, label: numberToChinese(k + 1) + this.$t('operation.gasPrice.ladderGas') }, price: { value: v, key: `price${k + 1}`, field: `price${k + 1}`, label: numberToChinese(k + 1) + this.$t('operation.gasPrice.ladderPrice') } }))
    return {
      initForm: {
        ...
        cycle: 12,
        heatingLadders: cloneDeep(laddres), // 采暖阶梯方案
        nonHeatingLadders: cloneDeep(laddres), // 非采暖阶梯方案
      ...
      formRules: {
        ...
        cycleStartTime: [
          { required: true, message: `${this.$t('selectHint')}${this.$t('operation.gasPrice.cycleStartTime')}`, trigger: 'blur' }
        ],
        heatingcycleStartTime: [
          { required: true, message: `${this.$t('selectHint')}${this.$t('operation.gasPrice.heatingcycleStartTime')}`, trigger: 'blur' }
        ],
        nonHeatingcycleStartTime: [
          { required: true, message: `${this.$t('selectHint')}${this.$t('operation.gasPrice.nonHeatingcycleStartTime')}`, trigger: 'blur' }
        ],
        settlementDay: [
  ...
  methods: {
    ...
    generateFormData(data) {
      ...
      if (data.isHeating) {
        deepCloneData.heatingLadders = ladders
        deepCloneData.heatingcycleStartTime = data.cycleStartTime
      } else {
        deepCloneData.nonHeatingLadders = ladders
        deepCloneData.nonHeatingcycleStartTime = data.cycleStartTime
      }
    ...
    analysisFormData(data) {
      const heatingPriceData = cloneDeep(data)
      const nonHeatingPriceData = cloneDeep(data)
      const heatingLadders = data.heatingLadders || []
      const nonHeatingLadders = data.nonHeatingLadders || []

      // 组装采暖阶梯价格信息
      heatingPriceData.isHeating = dataStatusField.VALID
      heatingPriceData.cycleStartTime = heatingPriceData.heatingcycleStartTime
      heatingLadders.forEach((ladder, index) => {
        const { price, gas } = ladder
        heatingPriceData[price.field] = price.value
        heatingPriceData[gas.field] = gas.value
      })
      delete heatingPriceData.heatingLadders

      // 组装非采暖阶梯价格信息
      nonHeatingPriceData.isHeating = dataStatusField.INVALID
      nonHeatingPriceData.cycleStartTime = nonHeatingPriceData.nonHeatingcycleStartTime
      nonHeatingLadders.forEach((ladder, index) => {
        const { price, gas } = ladder
        nonHeatingPriceData[price.field] = price.value
        nonHeatingPriceData[gas.field] = gas.value
      })
      delete nonHeatingPriceData.nonHeatingLadders
      // console.log('deepCloneData', deepCloneData)
      return [heatingPriceData, nonHeatingPriceData]
    },
    ...
    saveFormData(data = null) {
      ...
      return new Promise((resolve) => {
        ...
        saveHeatingPrice(data, data?.id || '').then((result) => {
    ...
    // 设置阶梯价格表单验证规则
    setLadderPriceRules(priceInfo, index, ladders) {
      ladders = ladders || this.form.laddres
      // console.log(priceInfo)
      // console.log(index)
      const { label, value } = priceInfo
      const rules = []
      rules.push({ pattern: positiveNumberReg, range: { min: 0 }, message: `${label}${this.$t('operation.gasPrice.integerHint')}` })
      if (index === 0) {
        rules.push({ required: true, message: `${label}${this.$t('operation.gasPrice.notEmptyHint')}`, trigger: 'blur' })
      } else {
        // 如果对应的阶梯气量不为空，那么阶梯单价也不能为空
        const currentGasValue = ladders[index]?.gas?.value || ''
    ...
    // 设置阶梯气量表单验证规则
    setLadderGasRules(gasInfo, index, ladders) {
      ladders = ladders || this.form.laddres
      ...
      if (index === 0) {
        ...
        const currentPriceValue = ladders[index]?.price?.value || ''
        if (currentPriceValue && !value) {
          rules.push({ required: true, message: `${label}${this.$t('operation.gasPrice.notEmptyHint')}`, trigger: 'blur' })
        }
        // 查找上一个阶梯信息
        const lastLadder = ladders[index - 1] || {}
        const lastGasValue = +(lastLadder?.gas?.value || '')
        const lastGasLable = lastLadder?.gas?.label || ''
        // 查找下一个阶梯信息
        const nextLadder = ladders[index + 1] || {}
    ...
    // 判断是否是采暖阶梯价格
    isHeatingPrice(value) {
      return +value === +dataStatusField.VALID
    },
    // 组装阶梯信息表单信息
    generateLadders(value) {
      return this.isHeatingPrice(value) ? this.form.heatingLadders : this.form.nonHeatingLadders
    },

    // 是否显示采暖价格或者非采暖价格方案
    showLadderPrice(value) {
      if (!this.onlyView) {
        return true
      } else {
        return (this.isHeatingPrice(value) && this.form.isHeating) || (!this.isHeatingPrice(value) && !this.form.isHeating)
      }
    }
  }
}
