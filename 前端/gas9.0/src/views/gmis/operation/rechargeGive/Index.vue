<!-- 充值赠送活动 -->
<template>
  <div class="qc-main rechargeGive-page">
    <el-card shadow="never" class="no-border">
      <div slot="header">
        <span class="card-title">{{ $t('operation.preferential.rechargeGive.title') }}</span>
      </div>
      <!-- 列表搜索区域 -->
      <search-box class="search-box operation-box">
        <div slot="left" class="">
          <el-form :ref="searchFormRef" :model="searchForm" :inline="true" class="qc-search-form">
            ...
            <el-form-item label="" prop="activityName">
              <el-input v-model="searchForm.activityName" clearable :placeholder="$t('operation.preferential.rechargeGive.activityName')">
              </el-input>
            </el-form-item>
            <!-- 操作 -->
            <el-form-item label="" prop="dataStatus">
              <el-button type="primary" name="rechargeGive-search" icon="el-icon-search" class="search-btn rechargeGive-search" @click.native="handleSearch">{{ $t('search') }}</el-button>
              <el-button type="warning" name="rechargeGive-search" icon="el-icon-refresh-left" class="search-reset-btn" @click.native="resetSearch(searchFormRef)">{{ $t('reset') }}</el-button>
            </el-form-item>
            <!-- <el-form-item label="">
        ...
        <div slot="right" class="operation-btn-box">
          <el-button v-has-permission="['rechargeGive:add']" class="add-btn" name="add-btn" type="success" @click.native="showFormDialog(formStatusField.ADD, null)">{{ $t('add') }}</el-button>
        </div>
      </search-box>
      <div>
        <!--数据-->
        ...
        >
          ...
          <el-table-column
            prop="activityName"
            :label="$t('operation.preferential.rechargeGive.activityName')"
          >
          </el-table-column>
          <!-- 活动场景 -->
          <!-- <el-table-column
            prop="activityScene"
            :label="$t('operation.preferential.rechargeGive.activityScene')"
          >
          ...
          <el-table-column
            ...
            :label="$t('operation.preferential.rechargeGive.activityMoneyTypeLabel')"
          >
            <template #default="scope">
          ...
          <!-- 开始时间 -->
          <el-table-column 
            :label="$t('operation.preferential.rechargeGive.startTime')" 
            prop="startTime"
          >
            <template #default="scope">
          ...
          <!-- 结束时间 -->
          <el-table-column 
            :label="$t('operation.preferential.rechargeGive.endTime')" 
            prop="endTime"
          >
            <template #default="scope">
          ...
          >
            <template #default="{ row }">
              <el-button v-has-permission="['rechargeGive:edit']" type="primary" name="row-edit" @click.native="showFormDialog(formStatusField.EDIT, row)">{{ $t('edit') }}</el-button>
              <!-- <el-button v-has-permission="['rechargeGive:delete']" type="danger" name="row-delete" @click.native="handleDelete(row)">{{ $t('delete') }}</el-button> -->
            </template>
          </el-table-column>
        </el-table>
        <!--分页-->
        ...
        <el-dialog
          width="600px"
          :title="currentRow ? $t('operation.preferential.rechargeGive.edit') : $t('operation.preferential.rechargeGive.add')"
          :visible.sync="formDialogVisible"
          @close="formDialogVisible = false"
        >
          <recharge-give-form :form-data="currentRow" @success="handleSaveSuccess"></recharge-give-form>
        </el-dialog>
      </div>
    </el-card>
  </div>
...
import RechargeGiveForm from './Form.vue'
...
export default {
  ...
  methods: {
    ...
    async handleDelete(row) {
      const confirmResult = await awaitToJs(this.$confirm(this.$t('operation.preferential.rechargeGive.deleteConfirm'), this.$t('common.tips'), {
        type: 'warning'
      }))
    ...
    async handleSwitchChange(val, row) {
      ...
      if (!dataStatus) {
        const confirmResult = await awaitToJs(this.$confirm(this.$t('operation.preferential.rechargeGive.disableConfirm'), this.$t('common.tips'), {
          type: 'warning'
        }))
        if (confirmResult.error) {
