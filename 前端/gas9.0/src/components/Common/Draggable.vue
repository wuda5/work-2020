<template>
  ...
  >
    <div v-if="resizable">
      <div v-for="$_handle in handles" :key="$_handle" class="handle" :class="'handle-' + $_handle" :style="{ display: enabled ? 'block' : 'none'}" @mousedown.stop.prevent="handleDown($_handle, $event)"></div>
    </div>
    <slot></slot>
  </div>
</template>
