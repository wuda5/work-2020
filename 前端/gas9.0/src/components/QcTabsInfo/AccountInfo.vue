    ...
    <div slot="default" class="frameBody">
      ...
      <el-card shadow="never" class="listItem" style="background:#f7f7f7 ">
        <el-form ref="form" label-position="top" class="jh-text">
          <el-form-item label="账户名">
            <p>{{ accountInfo.customerName || '---' }}</p>
          </el-form-item>
          <el-form-item label="账户余额">
            <p style="color: deeppink">{{ accountInfo.accountMoney || '---' }}</p>
          </el-form-item>
        </el-form>
      </el-card>
      <!--list-->
      <template v-if="serialList.length">
        <el-card v-for="(item,index) in serialList" :key="index" shadow="never" class="listItem">
          <div slot="header">
            <el-icon name="time" />
            缴费时间：{{ item.createTime | parseTime }}
          </div>
          <el-form ref="form" label-position="top" class="jh-text">
            <el-form-item label="操作类型">
              <el-input :value="dictMap.ACCOUNT_SERIAL_SCENE[item.billType]" />
            </el-form-item>
...
export default {
  ...
  data() {
    return {
      ...
      customerCode1: this.customerCode,
  ...
  methods: {
    ...
    getAccountInfo() {
      ...
      tabsApi.accountInfo(params).then(res => {
    ...
    getAccountSerial() {
      ...
      tabsApi.accountSerial(this.searchData).then(res => {
        const {isSuccess, data} = res.data
        if (isSuccess) {
